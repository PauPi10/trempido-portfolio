...
import { OG_IMAGE_URL, ORG_ID } from "@/lib/kari-ld"

const SHOTS = [
  { src: "/images/kari/shots/en-1.webp", alt: "Kari App Store screenshot: “Master your social skills”, with the skill tiles Charisma, Influence, Confidence, Boundaries and Connect with anyone." },
  { src: "/images/kari/shots/en-2.webp", alt: "Kari App Store screenshot listing the training areas: dating, social confidence, negotiation, charisma, leadership, public speaking, boundaries, storytelling, persuasion." },
  { src: "/images/kari/shots/en-3.webp", alt: "Kari App Store screenshot: “Meet your personal social coach”, a live voice conversation with Kari and her instant feedback." },
]

const SHOTS_LD = {
  "@context": "https://schema.org",
  "@type": "ItemList",
  "@id": `${KARI.siteUrl}/#screenshots`,
  name: "Kari app screenshots",
  itemListElement: SHOTS.map((s, i) => ({
    "@type": "ImageObject",
    position: i + 1,
...
const VIDEO_LD = {
  "@context": "https://schema.org",
  "@type": "VideoObject",
  "@id": `${KARI.siteUrl}/#coach-video`,
  name: "Meet Kari, the AI charisma coach",
  description: "A short loop of Kari, the coach character from the app that trains charisma and social skills by spoken conversation.",
  thumbnailUrl: [`${KARI.siteUrl}/images/kari/char/kari.png`, OG_IMAGE_URL],
  contentUrl: `${KARI.siteUrl}/images/kari/kari-loop.mp4`,
  uploadDate: KARI.datePublished,
  duration: "PT4S",
  width: 720,
  height: 720,
  inLanguage: "en",
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://karicoach.app",
    languages: { en: "https://karicoach.app", es: "https://karicoach.app/es", "x-default": "https://karicoach.app" },
  },
}
  ...
  return (
    <>
      ...
      <main>
            ...
            <Reveal delay={0.08}>
              <div className="k-catart" style={{ marginTop: 40 }}>
                {ROLEPLAY_CATEGORIES.map((c) => (
                  <Link key={c.id} href={`/roleplays/${c.id}`} className="k-catart__item" style={{ textDecoration: "none" }}>
        ...
        {/* ─── SCREENSHOTS ──────────────────────────────────── */}
        <section className="k-section k-section--tight" style={{ background: "var(--k-paper-deep)" }} aria-label="App screenshots">
          <div className="k-wrap">
            <Reveal>
              <span className="k-eyebrow">Inside the app</span>
              <h2 className="k-h2" style={{ marginTop: 14, maxWidth: "22ch" }}>
                What a session <span className="k-italic">looks like</span>
              </h2>
            </Reveal>
            <div className="k-grid-3" style={{ marginTop: 36, alignItems: "start" }}>
              {SHOTS.map((sh, i) => (
                <Reveal key={sh.src} delay={i * 0.08}>
            ...
            <Reveal>
              <a className="k-btn k-btn--ghost" href={KARI.appStoreUrl} target="_blank" rel="noopener" style={{ marginTop: 30 }}>
                See it on the App Store <ArrowRight size={17} />
              </a>
            </Reveal>
          </div>
        </section>
        ...
        {/* ─── MEET KARI (video) ────────────────────────────── */}
        <section className="k-section k-section--tight" aria-label="Meet Kari">
          <div className="k-wrap">
            <div className="k-grid-2" style={{ alignItems: "center", gap: "clamp(30px,4vw,60px)" }}>
              <Reveal>
                <video
                  src="/images/kari/kari-loop.mp4"
                  poster="/images/kari/char/kari.png"
                  autoPlay
                  muted
                  loop
                  playsInline
                  width={720}
                  height={720}
                  aria-label="Kari, the coach, in the app's animated loop"
                  style={{ width: "min(100%, 360px)", height: "auto", borderRadius: 28, boxShadow: "var(--k-shadow)", display: "block", marginInline: "auto" }}
                />
              </Reveal>
              <Reveal delay={0.1}>
                <span className="k-eyebrow">Your coach</span>
                <h2 className="k-h2" style={{ marginTop: 14 }}>
                  Meet <span className="k-italic">Kari</span>
                </h2>
                <p className="k-lead" style={{ marginTop: 18 }}>
                  She teaches one technique, listens to you try it, and corrects one thing while quoting your literal
                  words. Then she sends you into a scene with a character who will not make it easy, and reads back
                  what she heard. No flattery, no “great job”. A count.
                </p>
                <Link className="k-btn k-btn--ghost" href="/how-it-works" style={{ marginTop: 26 }}>
                  How a session works <ArrowRight size={17} />
                </Link>
              </Reveal>
            </div>
          </div>
